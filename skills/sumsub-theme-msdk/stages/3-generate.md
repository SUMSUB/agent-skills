...
- **Classic project:** register it explicitly — the canonical recipe lives in [`../../sumsub-integrate-msdk/references/pbxproj-editing.md`](../../sumsub-integrate-msdk/references/pbxproj-editing.md) (prefer the Ruby `xcodeproj` gem; to hand-edit, add the four pbxproj entries mirroring a sibling `.swift` file, then validate with `plutil -lint`). `SumsubTheme.swift` registers exactly the same way as any source file.
